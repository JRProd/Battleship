...
public class ShotSelectiveLowestClusterAI extends Player{
	
	//Cluster board
	private int[][] shotPerCluster = new int[Constants.CLUSTERS][Constants.CLUSTERS];
	...
	{
		//Find the lowest cluster
		int coords[] = new int[2];
		int chosenCluster[] = {0,0};
		int minClusterVal = shotPerCluster[chosenCluster[0]][chosenCluster[1]];
		
		//Brute force search for lowest cluster
		for(int i = 0; i < Constants.CLUSTERS; i++)
		{
			for(int j = 0; j < Constants.CLUSTERS; j++)
			{
				if(minClusterVal > shotPerCluster[i][j])
				{
					chosenCluster[0] = i;
					chosenCluster[1] = j;
					minClusterVal = shotPerCluster[i][j];
				}
			}
		}
		
		//Define the cluster break up
		int clusterBreakup = (int) Math.ceil((double)Constants.SIZE / Constants.CLUSTERS);
		
		//Get random cords for 
		Random randomCoord = new Random();
		int xCoord = randomCoord.nextInt(clusterBreakup);
		int yCoord = randomCoord.nextInt(clusterBreakup);
		
		
		//Get cords from cluster 
		coords[0] = clusterBreakup * chosenCluster[0] + xCoord;
		coords[1] = clusterBreakup * chosenCluster[1] + yCoord;
		
		//If out of bound force to bounds
		if(coords[0] >= Constants.SIZE)
			coords[0] = Constants.SIZE -1;
		if(coords[1] >= Constants.SIZE)
			coords[1] = Constants.SIZE -1;
		
		//Debug
		if(Constants.DEBUG)
			System.out.println("LOWEST_CLUSTER Chosen Cluster :: " + chosenCluster[0] + " " + chosenCluster[1] 
					+ " with score of " + shotPerCluster[chosenCluster[0]][chosenCluster[1]]);
		
		//return coords
		return coords;
	}
	
	private int[] getShipCoords()
	{
	...
	public int[] shoot() {
		...
		if(Constants.DEBUG)
		{
			...
			System.out.println("Adding one to shotPerCluster[" + coords[0] / (Constants.SIZE/Constants.CLUSTERS) 
					+ "][" + coords[1] / (Constants.SIZE/Constants.CLUSTERS) + "]");
		}
		if(coords[0] >= 0 && coords[0] < Constants.SIZE && coords[1] >= 0 && coords[1] < Constants.SIZE)
			shotPerCluster[coords[0] / (Constants.SIZE/Constants.CLUSTERS)][coords[1] / (Constants.SIZE/Constants.CLUSTERS)] ++;
